...
- **[new]** **Admin hot reload for providers**: saving `providers.json` through `/admin/api/generic-providers` now refreshes the in-memory provider registry and runtime map immediately; newly added providers appear in `/admin/api/providers`, `/admin/api/codex-targets`, and `/v1/models` without a process restart when their key is configured.
- **[new]** **Service-level upstream key API**: new `PUT /admin/api/service-provider-runtime/:providerId` writes the shared provider API key and optional base URL into `<dataDir>/.env`, updates `process.env`, and hot-reloads runtimes. This is the operator-level counterpart to per-user BYOK (`/admin/api/me/upstream-keys`) and is the endpoint external service managers should call when they want provider models to be advertised globally.
- **[new]** **One-shot provider save with service key**: `PUT /admin/api/generic-providers` can now accept an inline `apiKey` / `serviceApiKey` on each submitted provider. The key is written to `<dataDir>/.env`, removed before `providers.json` is persisted, and hot-reloaded in the same request so external managers can add a provider and make its models visible in `/v1/models` as one operation.
