...
- **[doc]** **Desktop win-arm64 is currently not built** (answering "check the Windows platforms too"): the CI matrix intentionally skips win-arm64 (better-sqlite3 v12.x win32-arm64 prebuilds are unreliable, and the existing `probe-prebuild.mjs` isn't a sound cross-arch gate). Windows-on-ARM users should use the x64 installer (it runs under Win11 ARM's x64 emulation). A known gap, now documented rather than silently missing.
