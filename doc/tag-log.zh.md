...
- **[opt]** **客户端请求的模型现在优先于运行时覆盖** —— 此前 webui 的运行时覆盖（`selectProvider` 的 Pass 0）**无条件**优先，会劫持掉 Codex 明确配置的模型（例如 Codex 发 `mimo-v2.5-pro`，覆盖却强制成 `deepseek`）。现在路由优先级为：**(1)** 客户端模型——只要有「已注册且有 key」的 provider 能识别它；**(2)** 运行时覆盖——当模型 id 不在任何 provider 目录里时；**(3)** 默认 provider 兜底。于是 Codex 明确配置的模型会被尊重，而覆盖退化为「未识别模型 id」时的智能兜底（不再静默改写成默认模型）。实现：`src/server.ts` 的 `selectProvider` 把覆盖块从 Pass 0 移到识别 pass 之后；管理台「仅运行时覆盖」说明同步更新。未设置覆盖时行为不变。
...
- **[fix]** **桌面端 Base URL 留空时上游主机变空**（启动 banner `upstream:` 空白、请求没有主机）：桌面端 Settings 的 Base URL 字段留空时，会把 `MIMO_BASE_URL=`（空串）写进 `.env`——这是**有意**的（清空字段=清掉该行，用于"清空 API key 停用某 provider"）。但配置解析用 `??` 取 base url，**空串不是 nullish、会"赢"过后面的回落链**，导致不再按 key 前缀自动选主机（`tp-` → `token-plan-cn.xiaomimimo.com` / `sk-` → `api.xiaomimimo.com`），`upstream` 变成空串、请求发不出去。现在把 env / CLI 传入的**空白或纯空格** base url 一律当作"未设置"，回落链恢复（key 前缀推断 → 默认主机）。已有 `.env` 里残留空 `MIMO_BASE_URL` 的用户**升级后无需重存即自动修复**。
